...
// Provider config matching `prompt.test.ts` but with an aggressively short
// chunkTimeout so Test A surfaces `SSEStallError` within the 4s budget instead
// of the production default (120s / 600s).
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* (input) {
        ...
        // Bounded wait for the retry transition. Budget covers: first setup
        // pass (cold provider state, models.dev load), one chunk timeout (1s),
        // plus schedule classification. If the fiber never transitions to
        // retry, the hang regression is back. `pollUnsafe` is the public
        // synchronous-peek API — we use it to short-circuit if the fiber dies
        // early so the error cause surfaces, rather than timing out blindly
        // at 12s.
        const observed = yield* Effect.gen(function* () {
          ...
          const snap = yield* sessionStatus.get(chat.id)
          if (snap.type === "retry") return snap
          return yield* Effect.fail(new Error(`expected retry status within 12s; last status: ${JSON.stringify(snap)}`))
        }).pipe(
          Effect.ensuring(
            Effect.gen(function* () {
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* (input) {
        ...
        // follow-up call fall through to the server's auto "ok"/stop response,
        // so no more queue entries are required.
        ...
        // F12: use the real production auto-reply path. Previously this test
        // mirrored SessionAutoReply's contract inline via manual
        ...
        const exit = yield* Fiber.await(fiber).pipe(
          Effect.timeoutOrElse({
            duration: "10 seconds",
            orElse: () =>
              Effect.die(new Error("root loop did not complete within 10s — subagent question likely deadlocked")),
          }),
        )
        ...
        // The rejection must propagate into the subagent's tool output so the
        // parent (task tool) sees the failure. Walk the root + child sessions
        // and locate the question tool part — it must be in error state with
        // the RejectedError message.
